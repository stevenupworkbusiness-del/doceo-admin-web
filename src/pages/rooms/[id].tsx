...
import FormEditModal from "@/models/FormEditModal";
...
const SingleRoom = () => {
  ...
  const handleOptionOnOfChange = async (e: any) => {
    setRefreshing(true);
    try {
  ...
  return (
    <>
      ...
      {chatClient && <AddDoctorModal
        show={isModalVisible}
        closeModal={closeModal}
        room={room}
      />}
      {chatClient && <AddCompanyModal
      ...
      >
            ...
            <div className="card shadow-none bg-transparent">
              <div className="card-body p-0">
                ...
                >
                  ...
                  >
                    <h1 className="absolute text-4xxl text-white">
                      {query.media === ""
                        ? "Chat"
                        : query.media === "record"
                          ? "Record"
        ...
        {query.media === "" ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              {refreshing ? (
                <div className="flex items-center justify-center h-10 w-20 mt-4">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div>
                </div>
              ) : (
                <select
                  onChange={(e) => {
                    handleOptionOnOfChange(e);
                  }}
                  value={value}
                  style={{
                    padding: "8px 12px",
            ...
            <div className="w-[90%] mx-auto pt-10">
              <StreamIntegrationTable
                selectedRoomId={query.id}
                setvalue={setValue}
        ...
        {showForm && (
          <FormModal
            selectedRoom={query.id as string}
            onHideModal={() => { setShowForm(false); }}
            value={value}
            setvalue={setValue}
          />
        )}
        {showEditForm && (
          <FormEditModal
            selectedRoom={query.id as string}
            onHideModal={() => setShowEditForm(false)}
            value={value}
            setvalue={
              setValue
            }
